    ...
    <section class="section contact">
        <div class="card p-4">
            <form method="POST" action="{{ route('event.update', ['event' => $event->id]) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT') <!-- Tambahkan method PUT untuk menandai bahwa ini adalah permintaan update -->
                ...
                <div class="mb-3">
                    ...
                    <textarea type="text" class="form-control" id="tanggal" name="tanggal" style="height: 80px">{{ $event->tanggal }}</textarea>
                </div>

                <div class="mb-3">
                    <label for="waktu" class="form-label">Waktu</label>
                    <textarea type="text" class="form-control" id="waktu" name="waktu" style="height: 80px">{{ $event->waktu }}</textarea>
                </div>

                <div class="mb-3">
                    <label for="lokasi" class="form-label">Lokasi</label>
                    <textarea type="text" class="form-control" id="lokasi" name="lokasi" style="height: 80px">{{ $event->lokasi }}</textarea>
                </div>

                <div class="mb-3">
                    <label for="link_pendaftaran" class="form-label">Link Pendaftaran</label>
                    <input type="text" class="form-control" id="link_pendaftaran" name="link_pendaftaran" value="{{ $event->link_pendaftaran }}">
                </div>

                <div class="mb-3">
                    <label for="tentang" class="form-label">Deskripsi Event</label>
                    <textarea type="text" class="form-control" id="tentang" name="tentang" style="height: 150px;">{{ $event->tentang }}</textarea>
                </div>

                <div class="mb-3">
                    <label for="hal" class="form-label">Hal Yang Dipelajari</label>
                    <textarea type="text" class="form-control" id="hal" name="hal" style="height: 150px;">{{ $event->hal }}</textarea>
                </div>

                <div class="mb-3">
                    <label for="narasumber" class="form-label">Narasumber</label>
                    <textarea type="text" class="form-control" id="narasumber" name="narasumber" style="height: 100px;">{{ $event->narasumber }}</textarea>
                </div>

                <div class="mb-3">
                    <label for="syarat" class="form-label">Syarat Pendaftaran</label>
                    <textarea type="text" class="form-control" id="syarat" name="syarat" style="height: 150px;">{{ $event->syarat }}</textarea>
                </div>

                <div class="mb-3">
                    <label for="biaya" class="form-label">Biaya</label>
                    <textarea type="text" class="form-control" id="biaya" name="biaya" style="height: 100px;">{{ $event->biaya }}</textarea>
                </div>
